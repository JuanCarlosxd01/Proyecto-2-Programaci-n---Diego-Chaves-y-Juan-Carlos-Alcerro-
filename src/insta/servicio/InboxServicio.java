...
public class InboxServicio {
    ...
    Respuesta ejecutar(String actor, Solicitud solicitud)
        ...
        if (solicitud.getOperacion()
                == Solicitud.Operacion.ELIMINAR_CONVERSACION) {

            a.usuario(otro);

            propios.eliminarSi(
                    mensaje -> mensaje.perteneceAConversacion(
                            actor,
                            otro
                    )
            );
            ...
            return Respuesta.exito(
                    "Conversación eliminada de tu historial."
            );
        }
        ...
        if (solicitud.getOperacion()
                == Solicitud.Operacion.CONVERSACION) {
        ...
        if (solicitud.getOperacion()
                == Solicitud.Operacion.LEER_CONVERSACION) {

            for (Mensaje mensaje : propios) {
                if (mensaje.recibidoPor(actor)
                        && mensaje.enviadoPor(otro)) {

                    mensaje.marcarComoLeido();

                    Mensaje espejo = ajenos.buscar(
                            elemento -> elemento.getId()
                                    .equals(mensaje.getId())
                    );

                    if (espejo != null) {
                        espejo.marcarComoLeido();
                    }
                }
            }
        ...
        Mensaje nuevo = solicitud.getOperacion()
                == Solicitud.Operacion.ENVIAR_MENSAJE
                ? Mensaje.crearTexto(
                        actor,
                        otro,
                        solicitud.arg(1)
        ...
        return Respuesta.exito("Mensaje enviado.")
                .conLista(resultado);
    }
}
