...
public class InstaServicio implements AutoCloseable {
    ...
    public InstaServicio() throws IOException, ArchivoCorruptoException {
        this("INSTA_RAIZ");
    }

    public InstaServicio(String raiz) throws IOException, ArchivoCorruptoException {
        ...
        try {
            if (!repo.existe("users.ins")) {
                try (DirectoryStream<Path> contenido =
                    ...
                    for (Path path : contenido) {
                        if (!path.getFileName().toString().equals("servidor.lock")) {
                ...
                repo.guardar(new RepositorioInsta.Cambio(
                        "users.ins", new ListaEnlazada<UsuarioInsta>()
                ));
            }
            ...
            repo.close();
            if (e instanceof ArchivoCorruptoException corrupto) throw corrupto;
            if (e instanceof IOException io) throw io;
            throw new IOException("No se pudo inicializar Instagram.", e);
        }
    }

    ListaEnlazada<UsuarioInsta> usuarios()
            throws IOException, ArchivoCorruptoException {
        ListaEnlazada<UsuarioInsta> lista = repo.leer(
                "users.ins", UsuarioInsta.class
        );
        Set<String> nombres = new HashSet<>();

        for (UsuarioInsta usuario : lista) {
            if (usuario.getUsername() == null
                    || !usuario.getUsername().matches("[a-z][a-z0-9_]{2,23}")
    ...
    static String normalizar(String valor) {
        return Objects.requireNonNull(valor, "Falta el username.")
    ...
    boolean visible(String nombre) throws IOException, ArchivoCorruptoException {
        UsuarioInsta usuario = buscarUsuario(nombre);
    ...
    void guardarUsuario(
        ...
        lista.eliminarSi(
                usuario -> usuario.getUsername().equals(actualizado.getUsername())
        );
        lista.agregar(actualizado);

        if (lista.size() > 100000) {
            throw new IllegalArgumentException("Límite de cuentas alcanzado.");
        }

        RepositorioInsta.Cambio[] lote = Arrays.copyOf(extras, extras.length + 1);
        lote[extras.length] = new RepositorioInsta.Cambio("users.ins", lista);
        repo.guardar(lote);
    }
    ...
    static void validarDatos(String nombre, char genero, int edad, String password) {
        ...
        if (password != null) validarPassword(password);
    }
    ...
    ) throws IOException, ArchivoCorruptoException, UsernameDuplicadoException {
    ...
    public synchronized UsuarioInsta registrarUsuario(
        ...
        if (!id.matches("[a-z][a-z0-9_]{2,23}")
                || id.matches("con|prn|aux|nul|com[1-9]|lpt[1-9]|stickers_globales")) {
            throw new IllegalArgumentException("Username inválido o reservado.");
        }

        if (buscarUsuario(id) != null) throw new UsernameDuplicadoException();

        if (repo.existe(id)) {
            throw new IOException("Ya existe una carpeta sin cuenta con ese nombre.");
        }

        var credenciales = SeguridadPassword.generarCredenciales(password);
        String rutaFoto = id + "/imagenes/perfil-" + UUID.randomUUID() + ".png";

        byte[] imagen = foto == null
                ? ImagenesInsta.dibujar(
                        id.substring(0, 1).toUpperCase(Locale.ROOT), 0x425b76
                )
                : ImagenesInsta.validar(foto);

        UsuarioInsta usuario = new UsuarioInsta(
                nombre.strip(), genero, id,
                credenciales.getPasswordHash(),
                credenciales.getPasswordSalt(), edad, rutaFoto
        );
        ...
        guardarUsuario(
                ...
                stickers.cambio(id, repo.leer("globales.ins", Sticker.class)),
                new RepositorioInsta.Cambio(rutaFoto, imagen),
                new RepositorioInsta.Cambio(id + "/folders_personales", Boolean.TRUE),
                new RepositorioInsta.Cambio(id + "/stickers_personales", Boolean.TRUE)
        );

        return usuario;
    }

    public synchronized UsuarioInsta iniciarSesion(String username, String password)
            throws IOException, ArchivoCorruptoException {
        if (username == null || password == null
                || username.length() > 24 || password.length() > 128) {
            return null;
        }

        UsuarioInsta usuario = buscarUsuario(username);
        return usuario != null && SeguridadPassword.verificar(
    ...
    public synchronized UsuarioInsta buscarUsuario(String username)
            throws IOException, ArchivoCorruptoException {
        String id = normalizar(username);
        return usuarios().buscar(usuario -> usuario.getUsername().equals(id));
    }

    public synchronized void exigirCuentaActiva(String username)
            throws IOException, ArchivoCorruptoException, CuentaDesactivadaException {
        if (!usuario(username).estaActiva()) throw new CuentaDesactivadaException();
    }

    public synchronized Respuesta operar(String actor, Solicitud solicitud)
            throws Exception {
        usuario(actor);
        ...
        if (solicitud.getOperacion() != Solicitud.Operacion.REACTIVAR
                && !perfilPropio) {
        ...
        return switch (solicitud.getOperacion()) {
            ...
            case PERFIL, EDITAR_PERFIL, FOTO_PERFIL, DESACTIVAR, REACTIVAR ->
            ...
            case ENVIAR_MENSAJE, ENVIAR_STICKER, CONVERSACION,
                    CONVERSACIONES, LEER_CONVERSACION,
                    ELIMINAR_CONVERSACION, NO_LEIDOS ->
                inboxServicio.ejecutar(actor, solicitud);
            case STICKERS, IMPORTAR_STICKER, CREAR_CARPETA, CARPETAS, ARCHIVO ->
                stickerServicio.ejecutar(actor, solicitud);
            default -> throw new IllegalArgumentException("Operación no soportada.");
        };
    }

    <T> Respuesta pagina(ListaEnlazada<T> lista, String desplazamiento) {
        int desde = Integer.parseInt(desplazamiento);
        if (desde < 0) throw new IllegalArgumentException("Página inválida.");
        ...
        for (T valor : lista) {
            ...
            if (pagina.size() == 100) break;
            pagina.agregar(valor);
        }

        return Respuesta.exito("Consulta completada.")
                .conLista(pagina).conTotal(lista.size());
    }

    private void ejemplos() throws Exception {
        String[] bases = {"noticias_demo", "deporte_demo", "cine_demo"};
        ...
        if (!repo.existe("semilla.ins")) {
            ...
            repo.guardar(new RepositorioInsta.Cambio("semilla.ins", nombres));
        }
        ...
        for (String id : nombres) {
            ...
            if (lista.isEmpty()) {
                lista.agregar(Publicacion.crearTexto(id, temas[indice % temas.length]));
                lista.agregar(Publicacion.crearTexto(
                        id, "Comparte tu mirada. " + temas[indice % temas.length]
                ));
