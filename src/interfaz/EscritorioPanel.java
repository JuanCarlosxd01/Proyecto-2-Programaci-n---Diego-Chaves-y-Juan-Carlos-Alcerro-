...
public class EscritorioPanel extends JPanel {
    ...
    private JButton iconoSeleccionado;
    ...
    public EscritorioPanel(
            JPanel contenedor, CardLayout transicion, GestorUsuarios gestorUsuarios
    ) {
    ...
    private JButton crearIcono(String nombre, String simbolo, int columna, int fila) {
        JButton icono = new JButton(
                "<html><center><span style='font-size:30px'>" + simbolo
                + "</span><br>" + nombre + "</center></html>"
        );
        ...
        icono.setBounds(
                columna * anchoCelda, fila * altoCelda, anchoCelda, altoCelda
        );
    ...
    private void detectarClicks(JButton icono, String nombre) {
        icono.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (e.getClickCount() == 2) abrirVentana(nombre);
            }
        });
    }
    ...
    private void abrirVentana(String nombre) {
        ...
        ventana.setSize(
                Math.max(700, escritorio.getWidth() - 100),
                Math.max(550, escritorio.getHeight() - 100)
        );
        ...
        if (nombre.equals("CMD")) {
            ...
            cmd.setAccionCerrar(ventana::dispose);
            ventana.add(cmd);
        } else if (nombre.equals("Word")) {
            EditorTextoPanel editor = new EditorTextoPanel();
            editor.setAccionCerrar(ventana::dispose);
            ventana.add(editor);
            ...
            if (!Sesion.esAdministrador()) {
                JOptionPane.showMessageDialog(
                        this, "Solo el administrador puede administrar usuarios.",
                        "Acceso denegado", JOptionPane.ERROR_MESSAGE
                );
                ventana.dispose();
                return;
            }
            ventana.add(new AdministrarUsuariosPanel(gestorUsuarios));
        } else if (nombre.equals("INSTA+")) {
            InstaPrincipalPanel insta = new InstaPrincipalPanel();
            insta.setAccionCerrar(ventana::dispose);
            ventana.add(insta);
            ...
            explorador.setAccionCerrar(ventana::dispose);
            ventana.add(explorador);

            explorador.setAbrirMusica(archivo -> {
                ReproductorPanel player = new ReproductorPanel();
                JInternalFrame marco = new JInternalFrame(
                        "Música", true, true, true, true
                );
                ...
                marco.addInternalFrameListener(new InternalFrameAdapter() {
                    @Override
                    public void internalFrameClosed(InternalFrameEvent e) {
                        player.cerrarReproductor();
                    }
                });

                escritorio.add(marco);
                crearBotonBarraTareas(marco, "Música");
                marco.setVisible(true);
                player.abrirArchivo(archivo);
            });
            ...
            File carpeta = new File(
                    "Z/" + Sesion.getUsuarioActual().getUsername() + "/Mis Imágenes"
            );
            VisorImagenPanel visor = new VisorImagenPanel(carpeta);
            visor.setAccionCerrar(ventana::dispose);
            ventana.add(visor);
        } else {
    ...
    private void activarAjusteVentana(JInternalFrame ventana) {
        ...
        Timer timer = new Timer(200, e -> {
            if (!ajustando[0]) ajustarVentanaAlBorde(ventana, ajustando);
        });
    ...
    private void ajustarVentanaAlBorde(JInternalFrame ventana, boolean[] ajustando) {
        ...
        if (x <= margen) {
            ...
            SwingUtilities.invokeLater(() -> ajustando[0] = false);
    ...
    private void mostrarOcultarMenuInicio() {
        if (menuInicio == null) return;
        menuInicio.setLocation(5, escritorio.getHeight() - menuInicio.getHeight());
        menuInicio.setVisible(!menuInicio.isVisible());
        if (menuInicio.isVisible()) escritorio.moveToFront(menuInicio);
    }
    ...
    private void mostrarConfirmacionApagar() {
        ...
        ventana.setBounds(
                (escritorio.getWidth() - 400) / 2,
                (escritorio.getHeight() - 180) / 2, 400, 180
        );
    ...
    private void cerrarSesion() {
        for (JInternalFrame ventana : escritorio.getAllFrames()) ventana.dispose();
